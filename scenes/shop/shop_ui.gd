...
func _ready() -> void:
	...
	_stash_panel.item_hovered.connect(func(item: ItemData, pos: Vector2) -> void:
		...
		_item_tooltip.show_for_item(item, null, inv, pos, _sell_price_for(item), "Sell"))
...
func _on_player_cell_clicked(grid_pos: Vector2i, button: int) -> void:
	...
	if _drag_state == DragState.DRAGGING:
		...
		match _drag_source:
			DragSource.MERCHANT:
				_complete_buy_to_player_grid(adjusted_pos, inv)
			DragSource.SOLD_PANEL:
				_complete_buyback_to_player_grid(adjusted_pos, inv)
