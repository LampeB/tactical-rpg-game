...
export class BattleScene extends Scene {
  constructor() {
    ...
    // Don't call initializeBattle() here - do it in onEnter()
  }

  initializeBattle() {
    // Set up player inventory (empty for now - will be copied from inventory scene)
    if (this.engine.scenes.has("inventory")) {
      const inventoryScene = this.engine.scenes.get("inventory");
      this.player.inventory = inventoryScene.inventoryGrid;
    } else {
  ...
  executeAITurn() {
    ...
    const availableSkills = enemy.aiSkills.filter(
  ...
  onEnter() {
    ...
    // Initialize battle when entering the scene
    ...
    this.buttons = [
      {
        text: "Menu",
        action: () => this.engine.switchScene("menu"),
        x: 50,
        y: 750,
        width: 80,
        height: 30,
  ...
  isPlayerTurn() {
    return (
      this.battleState === "selecting" && this.getCurrentActor() === this.player
    );
  }
  ...
  render(ctx) {
    ...
    ctx.textAlign = "center";
    ctx.fillText(
      "Click skills then targets | A: quick attack | H: quick heal | Space: skip | M: menu",
      this.engine.width / 2,
    ...
    ctx.textAlign = "center";
    ctx.fillText(
      "Click skills then targets | Use buttons to navigate | Auto Battle available",
      this.engine.width / 2,
      this.engine.height - 10
    );
  }
  ...
  renderPlayer(ctx) {
    ...
    if (this.selectedSkill) {
      ...
      ctx.textAlign = "left";
      ctx.fillText(
        `Selected: ${this.selectedSkill.name}`,
        x + 10,
        y + height + 15
      );
    }
  ...
  renderSkillPanel(ctx) {
    ...
    if (skills.length === 0) {
      ...
      ctx.font = "14px Arial";
      ctx.fillText(
        "No skills - configure inventory first",
        panelX + 10,
        panelY + 50
      );
      return;
    ...
    skills.forEach((skill, index) => {
      ...
      ctx.font = "10px Arial";
      ctx.fillText(
        `${skill.damage} DMG | ${skill.cost} MP | ${skill.type}`,
        panelX + 15,
        buttonY + 28
      );
    });
  ...
  renderBattleLog(ctx) {
    ...
    this.battleLog.forEach((message, index) => {
      const alpha = Math.max(
        0.4,
        1 - (this.battleLog.length - index - 1) * 0.1
      );
